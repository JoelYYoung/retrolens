...
# FlowCraft Distill — Conversation Log Analysis Skill
...
flowcraft-distill --version  # verify
```
...
4. **Write YAML DSL** to `.flowcraft/<name>.workflow.yaml`:
   ```yaml
   workflow:
     name: "my-workflow"
...
3. **Write output** to `.flowcraft/LESSONS.md`
...
```bash
flowcraft-distill scan --limit 10 --json
...
6. **Check existing artifacts** with `flowcraft-distill show` before writing
...
```bash
flowcraft-distill --skill-path  # prints path to bundled SKILL.md with full reference
```
