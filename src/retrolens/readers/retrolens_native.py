"""
RetroLens native log reader.

Reads session logs stored in the RetroLens format:
  logs/
...
def _extract_tool_calls_native(response: dict) -> list[models.ToolCallDetail]:
    """Extract tool calls from a RetroLens native response."""
    raw = response.get("raw_response", {})
    content = raw.get("content", [])
    tools = []
    idx = 0
...
# ── RetroLensNativeReader ──────────────────────────────────────────────────

class RetroLensNativeReader(BaseReader):
    """Read RetroLens native session logs."""

    source_type = "retrolens"
    ...
    def scan(self, path: Path | None = None) -> list[models.SessionInfo]:
        ...
        for entry in self._load_index():
            ...
            sessions.append(models.SessionInfo(
                ...
                title="",  # RetroLens native doesn't have titles
